...
1. Install XivBlend 0.0.10 or newer and select Blender in XivBlend's **Export** tab if it was not detected automatically.
...
Both fitting controls preserve the camera lens, current frame, animation, and playback state. They ignore the generated studio scenery, hidden objects, dummy meshes, and custom bone widgets when measuring the character. Existing compatible XivBlend `.blend` files gain these controls after the panel is reinstalled; the refined physical studio itself requires a new export with XivBlend 0.0.10 or newer.
