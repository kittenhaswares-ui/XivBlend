...
png_custom_vertical_array_definitions = {
    'chara_tile_norm_array': ArrayDefinition(
        'array_textures/chara/common/texture/tile_norm_array', r'tile_norm_array\..+\.vertical\.png$'
    ),
    'chara_tile_orb_array': ArrayDefinition(
        'array_textures/chara/common/texture/tile_orb_array', r'tile_orb_array\..+\.vertical\.png$'
    ),
    'bgcommon_detail_n_array': ArrayDefinition(
        'array_textures/bgcommon/nature/detail/texture/detail_n_array', r'detail_n_array\..+\.vertical\.png$'
    ),
    'bgcommon_detail_d_array': ArrayDefinition(
        'array_textures/bgcommon/nature/detail/texture/detail_d_array', r'detail_d_array\..+\.vertical\.png$'
    ),
}


# TextureNodeConfig defaults are Non-Color / CHANNEL_PACKED / Linear / REPEAT;
# entries only state deviations from those defaults.
texture_node_configs: dict[str, Union[TextureNodeConfig, ConditionalTextureConfig]] = {
    'chara_tile_norm_array': TextureNodeConfig(interpolation='Closest'),
    'chara_tile_orb_array': TextureNodeConfig(interpolation='Closest'),
    'bgcommon_detail_n_array': TextureNodeConfig(interpolation='Closest'),
    'bgcommon_detail_d_array': TextureNodeConfig(interpolation='Closest'),
    # Default config with conditional override for skin face materials
    'g_SamplerDiffuse_PngCachePath': ConditionalTextureConfig(
        default=TextureNodeConfig(colorSpace='sRGB'),
        overrides=[
            (
                ...
                TextureNodeConfig(colorSpace='sRGB', extension='EXTEND'),
            )
        ],
    ),
    'g_SamplerSkinDiffuse_PngCachePath': TextureNodeConfig(colorSpace='sRGB'),
    ...
    'g_SamplerIndex_PngCachePath': TextureNodeConfig(interpolation='Closest'),
    'Decal_PngCachePath': TextureNodeConfig(extension='CLIP'),
    ...
    'g_SamplerColorMap0_PngCachePath': TextureNodeConfig(colorSpace='sRGB'),
    'g_SamplerColorMap1_PngCachePath': TextureNodeConfig(colorSpace='sRGB'),
    ...
    'g_Sampler0_PngCachePath': TextureNodeConfig(colorSpace='sRGB'),
    'g_Sampler1_PngCachePath': TextureNodeConfig(colorSpace='sRGB'),
}
...
node_group_configs = {
    ...
    'meddle charactertattoo.shpk': [ColorMapping('OptionColor', 'OptionColor')],
    'meddle iris.shpk': [
    ...
    "meddle bg.shpk": [
        ...
        MaterialKeyMapping(
            'GetValues', 'GetAlphaMultiValues', 'GetAlphaMultiValues', True
        ),  # GetAlphaMultiValues2, GetAlphaMultiValues3 used in bguvcroll/lightshaft
...
ramp_lookups = {
    ...
    "MetalnessRoughnessGlossSpecularA": PackedColorTableRampLookup(
        [("Metalness", "Float"), ("Roughness", "Float"), ("GlossStrength", "Float"), ("SpecularStrength", "Float")],
        False,
    ),
    "MetalnessRoughnessGlossSpecularB": PackedColorTableRampLookup(
        [("Metalness", "Float"), ("Roughness", "Float"), ("GlossStrength", "Float"), ("SpecularStrength", "Float")],
        True,
    ),
    # "AnisotropyRampA": ColorTableRampLookup("Anisotropy", "Float", False),
    # "AnisotropyRampB": ColorTableRampLookup("Anisotropy", "Float", True),
    "SheenPropertiesA": PackedColorTableRampLookup(
        [("SheenRate", "Float"), ("SheenTint", "Float"), ("SheenAptitude", "Float")], False
    ),
    "SheenPropertiesB": PackedColorTableRampLookup(
        [("SheenRate", "Float"), ("SheenTint", "Float"), ("SheenAptitude", "Float")], True
    ),
...
@profile_function
def apply_material(
    material: bpy.types.Material, slots: List[bpy.types.MaterialSlot]
...
def setPngConfig(material: bpy.types.Material, cache_directory: str):
    ...
    for node in texture_nodes:
        ...
        # Unconfigured labels fall back to the TextureNodeConfig defaults
        config = resolve_config(label) or TextureNodeConfig()
