...
class RunAtlas(Operator):
    ...
    def execute(self, context):
        ...
        for group_idx, material_group in enumerate(material_groups):
            ...
            atlas_name = (
                f"Atlas_{joined_mesh.name}_{group_idx}" if len(material_groups) > 1 else f"Atlas_{joined_mesh.name}"
            )
            ...
            material_info.append(
                {
                    'index': mat_idx,
                    'material': material,
                    'width': actual_tex_width,
                    'height': actual_tex_height,
    ...
    def update_skyline(self, skyline, x, y, width, height):
        ...
        while i < len(skyline):
            ...
            if seg['x'] < x:
                new_skyline.append({'x': seg['x'], 'y': seg['y'], 'width': x - seg['x']})

            # Add right part if exists
            if seg_right > rect_right:
                new_skyline.append({'x': rect_right, 'y': seg['y'], 'width': seg_right - rect_right})

            i += 1

        # Add the new segment on top of the placed rectangle
        new_skyline.append({'x': x, 'y': new_y, 'width': width})
    ...
    def create_texture_atlas(self, context, joined_mesh, atlas_name):
        ...
        material_uv_mapping = self.copy_textures_to_atlas(
            materials, material_info, atlas_layout, atlas_images, used_texture_types
        )
    ...
    def create_atlas_images(self, atlas_name, atlas_width, atlas_height, texture_types):
        ...
        for tex_type in texture_types:
            ...
            atlas_image = bpy.data.images.new(name=atlas_image_name, width=atlas_width, height=atlas_height, alpha=True)
            ...
            for tex_type in texture_types:
                ...
                try:
                    ...
                    self.copy_texture_to_atlas(
                        source_image, atlas_buffers[tex_type], tile_x, tile_y, tile_w, tile_h, tex_type, alpha_img
                    )
    ...
    def copy_texture_to_atlas(
        self, source_image, atlas_pixels, dest_x, dest_y, width, height, tex_type, alpha_image=None
    ):
        ...
        if tex_type == 'alpha':
            ...
            logger.info("Packing alpha channel into diffuse texture")
            alpha_pixels = bake_utils.img_as_nparray(alpha_image)

            if alpha_pixels.shape[:2] != source_pixels.shape[:2]:
                alpha_pixels = self.bilinear_resize(
                    alpha_pixels, source_width, source_height, alpha_image.size[0], alpha_image.size[1]
                )
        ...
        if source_width != width or source_height != height:
            ...
            if resize_type == "Upscaling":
                logger.warning("Upscaling texture - may indicate packing inefficiency")
        ...
        if x1 > x0 and y1 > y0:
            ...
            atlas_pixels[dest_y + sy0 : dest_y + sy0 + (y1 - y0), x0:x1, :] = source_pixels[
