...
> **Prototype status:** version 0.0.14 turns each clicked body emote into an on-demand bundle: the primary PAP, exact TMB-scheduled facial clips, real TMB-spawned prop models, and native AVFX metadata share the game's 30 fps clock. Props use the locally installed game model, material, textures, colorset, hand, scale, and lifetime. New character exports preserve the original glTF socket basis so supported item props follow FFXIV's attachment orientation; older XivBlend files receive a deliberately narrow known-socket compatibility correction. Native `.avfx` sources are cached with their exact timing and placement, and embedded static geometry now receives a neutral unlit, double-sided inspection material instead of Blender's black metallic default. Effects that require FFXIV's Apricot particle runtime are still identified rather than replaced by fake Blender objects. A selected Penumbra animation mod can also contribute active overrides and standalone player pose/loop PAPs as clearly labeled **Custom** cards. The shared cache remains outside every `.blend`, and preview Actions/props are removed before saving.
...
- A Blender 5.x scene with a clean stick rig, category-first collections such as `Hair - <Mod Name>` and `Upper Body - <Mod Name>`, a 1440×1800 portrait camera, grounded seamless backdrop, softbox-style three-point lighting, mapped FFXIV materials, packed images, redacted provenance, an embedded build report, and a script-free A-pose/captured-pose Timeline slider. Collection organization does not rename the imported objects or mesh datablocks.
...
- With animation-browser version 0.6.0 installed, open the 3D View's `N` sidebar and expand **XivBlend** → **Render Studio**. Use **Smooth Animation** for a deliberately simplified clay viewport driven by one shared shader, then **Full Detail** for the exact exported appearance. F12/Render Portrait output and saving automatically switch to Full Detail and safely resume Smooth Animation afterward. On the validated 61-mesh c0801 export, Smooth Animation sustained 30 fps at a 1845×1158 Rendered viewport; results still depend on the scene and other GPU workloads.
...
- Native glTF rest matrices, bind transforms, and bone axes are preserved; the pose slider does not remap the rig. Version 0.0.14 additionally records the original source basis for attachment bones, allowing browser 0.6.0 to correct Blender's glTF display-axis remap before applying an item's ATCH transform. Older metadata-bearing XivBlend files get a fallback only for the known hand/throw sockets; arbitrary rigs and unusual attachment classes are not guessed. For an FBX round trip, use Primary Bone Axis `X` and Secondary Bone Axis `Y` at the FBX export/import boundary only.
- Existing `.blend` files are not retroactively relit or repaired for unnecessary opaque dither. Export again with 0.0.11 or newer for the new studio and material cleanup, and with 0.0.14 or newer for category/mod collections and exact saved socket bases. The 0.6.0 viewport, animation-bundle, camera, and render controls work on compatible metadata-bearing XivBlend files after reinstalling the Blender panel.

## Verified so far

- The Dalamud API 15 / .NET 10 release build completes with zero compilation errors; an offline NuGet vulnerability-feed warning may still appear when nuget.org is unavailable.
...
- Synchronized animation-bundle loading, timed facial NLA layers, exact apple import/material binding, transient cleanup, and Action-slot binding pass headless tests in Blender 5.0 and Blender 5.2. Catalog and request-queue manifests remain schema 2; animation bundles use schema 3.
...
- TMB-spawned model props are exact local game assets. Supported item props use the saved original glTF socket basis plus the game's ATCH transform, and older XivBlend exports have a compatibility correction for known hand/throw sockets. Separate spawned-object animation, physics, unusual attachment behavior, and other item classes may still need event-specific support. Native AVFX sources and placement metadata are extracted, but Blender does not yet simulate Apricot emitters, particles, trails, distortion, collision, or game shaders; the cache-v2 unlit/double-sided static geometry is inspection data, not equivalent playback. Custom face-PAP replacements and mod-supplied VFX assets are not imported yet.
...
The 0.0.14 compatibility line is Blender browser 0.6.0, character builder 0.9.0, catalog/request schema 2, and animation bundle schema 3.
