...
class CreateCopyForBaking(Operator):
    ...
    def execute(self, context):
        try:
            ...
            self.report(
                {'INFO'}, f"Copy complete! Created collection: {collection_name} with {len(mesh_copies)} mesh(es)"
            )
            return {'FINISHED'}
    ...
    def duplicate_armature_and_meshes(self, context, armature, mesh_objects, materials, bake_collection):
        ...
        self.report({'INFO'}, "  Joining and processing meshes...")
        joined_meshes = []
    ...
    def ensure_uv_layer(self, mesh_obj, uv_layer_name="UVMap"):
        ...
        try:
            bpy.ops.uv.smart_project(
                angle_limit=66.0, island_margin=0.02, area_weight=0.0, correct_aspect=True, scale_to_bounds=False
            )
