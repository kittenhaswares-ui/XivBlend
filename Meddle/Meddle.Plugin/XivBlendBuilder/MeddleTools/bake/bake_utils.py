...
def create_bake_image(
    name, width, height, background_color=(0.0, 0.0, 0.0, 1.0), alpha_mode='STRAIGHT', colorspace='sRGB'
):
    ...
    if tex_type in socket_mapping:
        for node in material.node_tree.nodes:
            if node.type != 'TEX_IMAGE' or not node.image:
                continue
            if any(link.to_socket.name == socket_mapping[tex_type] and link.from_node == node for link in links):
                return node.image
    elif tex_type == 'normal':
        for node in material.node_tree.nodes:
            if node.type != 'TEX_IMAGE' or not node.image:
                continue
            if any(link.to_node.type == 'NORMAL_MAP' and link.from_node == node for link in links):
                return node.image
