...
class RunBake(Operator):
        ...
        try:
            ...
            for material_name, material in materials.items():
                ...
                meshes_using_material = [
                    mesh for mesh in mesh_objects if material.name in [mat.name for mat in mesh.data.materials if mat]
                ]
                ...
                if "MeddlePackedUVs" in mesh_to_bake.data.uv_layers:
                    ...
                    logger.info(
                        f"Using MeddlePackedUVs layer for baking material {material.name} on mesh {mesh_to_bake.name}"
                    )
                else:
                ...
                self.update_progress(
                    context, progress, f"  Baking material {current_material}/{total_materials}: {material_name}..."
                )
    ...
    def bake_material(self, context, material, joined_mesh, uv_layer_name):
        ...
        max_image_size = (
            ...
            if settings
            else bake_utils.determine_largest_image_size(material)
        )
        ...
        for node_key, factory_func, location_offset, requires_pass in bake_config['special_nodes']:
            ...
            location = (duplicate_node.location.x + location_offset[0], duplicate_node.location.y + location_offset[1])
    ...
    def bake_pass(self, context, material, mesh, bake_name, max_image_size, uv_layer_name):
        ...
        def create_bake_image(bake_name, background_color, width, height, alpha_mode, colorspace):
            ...
            image = bpy.data.images.new(name=file_name, width=width, height=height, alpha=True)
        ...
        image = create_bake_image(
            bake_name, background_color, max_image_size[0], max_image_size[1], alpha_mode, colorspace
        )
        ...
        def apply_remapping(node, conn):
            ...
            if target_socket:
                if transform_func:
                    ...
                    logger.info(
                        f"Remapped {conn['input_name']} to {target_socket_name} via transform for {bake_name} bake"
                    )
                else:
                    # Direct connection
    ...
    def ensure_uv_layer(self, mesh_obj, uv_layer_name="UVMap"):
        ...
        try:
            ...
            # )
            bpy.ops.uv.unwrap(method='ANGLE_BASED', margin=0.02, fill_holes=True, margin_method='SCALED')
