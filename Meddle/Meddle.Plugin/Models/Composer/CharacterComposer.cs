...
public class CharacterComposer
{
    ...
    /// <summary>
    /// Machine-readable outcome for the one-click exporter.  Upstream Meddle is
    /// intentionally best-effort and logs per-part failures; XivBlend needs to
    /// distinguish a complete character from a usable but partial scene.
    /// </summary>
    public sealed class CompositionReport
    {
        ...
        {
            if (!Failures.Contains(message, StringComparer.Ordinal))
            {
    ...
    private void HandleModel(ParsedCharacterInfo characterInfo, ParsedModelInfo m, SceneBuilder scene, SkinningContext skinningContext)
    {
        if (!m.Enabled && exportConfig.ApplyVisibilityFlags)
        {
            ...
            Report.DisabledModelsSkipped++;
            return;
        }

        Report.ExpectedVisibleModels++;
    ...
    private (List<BoneNodeBuilder> bones, BoneNodeBuilder root)? ComposeCharacterInfo(ParsedCharacterInfo characterInfo, (ParsedCharacterInfo Owner, List<BoneNodeBuilder> OwnerBones, ParsedAttach Attach)? attachData, SceneBuilder scene, NodeBuilder root, ExportProgress rootProgress)
    {
        ...
        catch (Exception e)
        {
            ...
            Report.AddFailure($"Skeleton could not be built: {e.Message}");
            return null;
        }
        ...
        if (attachData != null)
        {
            ...
            catch (Exception e)
            {
                ...
                Report.AddFailure($"Attachment could not be connected to the rig: {e.Message}");
            }
        }
        else if (characterInfo.Attach.ExecuteType != 0)
        {
            ...
            catch (Exception e)
            {
                ...
                Report.AddFailure($"Root attachment could not be connected to the rig: {e.Message}");
            }
        }
        ...
        foreach (var t in characterInfo.Models)
        {
            ...
            catch (Exception e)
            {
                ...
                Report.AddFailure($"Model {t.Path.GamePath} failed: {e.Message}");
            }
        ...
        foreach (var t in characterInfo.Attaches)
        {
            ...
            catch (Exception e)
            {
                ...
                Report.AddFailure($"Attached character part failed: {e.Message}");
            }
            finally
            {
